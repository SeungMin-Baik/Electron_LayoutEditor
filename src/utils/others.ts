...
export function formatBytes(bytes: number, decimals?: number): string {
    ...
    return parseFloat((bytes / Math.pow(kilo, unitIndex)).toFixed(dm))
        + ' '
        + sizes[unitIndex];
}
...
export function MD5(string: string) {
        ...
        // return resolve(temp.toLowerCase());
        return temp.toLowerCase();
    }

    export function decrypt(key: any, text: any) {
        // const iv = '';
        // const crypto = require('crypto');
        // const cipher = crypto.createDecipheriv('aes-128-ecb', key.toString(), iv);
        // let crypted = cipher.update(JSON.stringify(text), 'base64', 'utf8');
        // crypted += cipher.final('utf8');
        // crypted = atob(crypted);
