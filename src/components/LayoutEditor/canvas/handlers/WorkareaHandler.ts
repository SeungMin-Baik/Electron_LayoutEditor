...
class WorkareaHandler {
    ...
    /**
     * Fit with workarea size
     *
     */
    public fitWithWrokareaSize = () => {
        const { workarea, canvas } = this.handler;

        const activeObject = canvas.getActiveObject();
        if (activeObject) {
            activeObject.set('scaleX', workarea.width / activeObject.width);
            activeObject.set('scaleY', workarea.height / activeObject.height);
    ...
    public calculateScale = (image?: FabricImage) => {
        ...
        const width = _element.width ? _element.width : workareaWidth;
        const height = _element.height ? _element.height : workareaHeight;
