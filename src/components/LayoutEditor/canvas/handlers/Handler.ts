...
import { imageThumbnail } from '@app/utils/main/thumbnail';
...
class Handler implements HandlerOptions {
     ...
     * @returns
     */
    public add = (obj: FabricObjectOption, centered = true, loaded = false, naturalSize?: any) => {
        ...
        if (obj.type === 'image') {
            if (naturalSize) {
                createdObj = this.addImage(newOption, naturalSize);
            } else {
            ...
            if (obj.type === 'textbox') {
                ...
                if (activeObject !== undefined) {
                    ...
                    if (activeObject['fontFamily'] === '') {
                        activeObject['fontFamily'] === 'Anton';
                    } else {
                        activeObject['fontFamily'] === obj.fontFamily;
                    }
     ...
     * @returns
     */
    public addImage = (obj: FabricImage, naturalSize?: any) => {
        ...
        if (obj.width) {
            imageObj.set({
                filters: [],
                width: naturalSize ? naturalSize.naturalWidth : image.naturalWidth,
                height: naturalSize ? naturalSize.naturalHeight : image.naturalHeight,
                scaleX: naturalSize ? obj.width / naturalSize.naturalWidth : obj.width / image.naturalWidth,
                scaleY: naturalSize ? obj.height / naturalSize.naturalHeight : obj.height / image.naturalWidth
            });
        }
    ...
    public importJSON = async (json: any, callback?: (canvas: FabricCanvas) => void) => {
        ...
        json.forEach((obj: FabricObjectOption) => {
            ...
            const targetObj = { ...obj };
            targetObj.width = obj.width * obj.scaleX;
            targetObj.height = obj.height * obj.scaleY;
            targetObj.scaleX = 1;
            targetObj.scaleY = 1;

            const naturalSize: any = {};
            if (obj.type === 'image') {
                const image = new Image();
                image.src = obj.src;
                image.addEventListener('load', () => {
                    naturalSize.naturalWidth = image.naturalWidth;
        ...
        this.canvas.zoomToPoint(new fabric.Point(this.canvas.width / 2, this.canvas.height / 2), this.canvas.getZoom() / 2);
        this.objects = this.getObjects();
        if (callback) {
    ...
    public editImportJSON = async (json: any, callback?: (canvas: FabricCanvas) => void) => {
        let tmpData = [];
        tmpData = json;
        ...
        .then(() => {
            setTimeout(() => {
                const objects = this.exportJSON().filter(obj => {
                    if (!obj.id) {
                        return false;
                    }
                    return true;
                ...
                for (let i = tmpSort.length - 1; i > 0; i--) {
                    const existObject = this.canvas.getObjects().filter((obj: any) => obj.id === tmpSort[i].id);
                    this.sendToBack(existObject[0]);
                }

                const existObject = this.canvas.getObjects();
                existObject.map((object: any) => {
    ...
    public toActiveSelection = (target?: FabricObject) => {
        console.log('toActivateSelection');
     ...
     * Send to back
     */
    public sendToBack = (value?) => {
        if (value) {
            this.canvas.sendToBack(value);
    ...
    public replaceImage = (prevImage, nextImage) => {

        const object = this.getObjectWithID(prevImage.assetid);

        const oriPositionInfo = {
            width: object.width,
        ...
        object.setSrc(electronConfig.APP.DIR_PATH.THUMBNAIL_PATH + '/' + nextImage.FileData.id + '_thumb.png', newObject => {
            if (!newObject) {
                throw (new Error('IMAGE_LOAD_FAILED'));
            }
            object.set({
                scaleX: (oriPositionInfo.width * oriPositionInfo.scaleX) / newObject.width,
                scaleY: (oriPositionInfo.height * oriPositionInfo.scaleY) / newObject.height,
            });
            this.canvas.renderAll();
        }, {
            name: nextImage.FileData.name,
            md5: nextImage.FileData.md5,
            assetid: nextImage.FileData.id,
            fileType: nextImage.FileData.fileType,
            mimeType: nextImage.FileData.mimeType,
        });
    }

    public getObjectWithID = (objectID: number): any => {
        let targetObject: any = {};
        const objects: Array<any> = this.canvas.getObjects();
        objects.map(object => {
            if (object && object.assetid === objectID) {
                targetObject = object;
            }
        });
